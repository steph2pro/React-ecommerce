...
function Button({ isLoading = false, isForm = false, onClick = undefined, isError = false, className, disabled = false, children }: PrimaryButton) {
  return (
        <button
            ...
            className={disabled ? "rounded-lg  bg-slate-200 hover:bg-gray-500 px-10 py-3 w-full text-white" : !isError ? " rounded-lg  w-full bg-custom_blue hover:bg-blue-700  px-10 py-3 text-white " + className : "  px-4 py-2 bg-green-950 rounded-lg w-full text-white " + className} >
